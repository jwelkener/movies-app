...
$(function () {
  ...
  $("tbody").on("click", ".btn.btn-danger", function (evt) {
    ...
    let indexToRemoveAt = allMovies.findIndex(movie => movie.currentId === +$(evt.target).data("deleteId"))

    // remove it from the array of movies
    allMovies.splice(indexToRemoveAt, 1)
  ...
  $(".fas").on("click", function (evt) {
    ...
    let sortedMovies = sortBy(allMovies, keyToSortBy, direction);
    ...
    for (let movie of sortedMovies) {
      const moreHTML = createMovieDataHTML(movie);
      $("#movie-table-body").append(moreHTML);
    }
